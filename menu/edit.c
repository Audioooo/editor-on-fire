...
int eof_menu_edit_paste_logic(int function)
{
	...
	//end arpeggio markers before the next note of the last note in an arpeggio
	long note = eof_get_track_size(eof_song, sourcetrack);
	if(eof_song->track[sourcetrack]->track_format == EOF_PRO_GUITAR_TRACK_FORMAT)
	{
		eof_log("ghey",1);
		for(j = 0; j < eof_song->pro_guitar_track[srctracknum]->arpeggios; j++)
		{	//For each arpeggio section in the active track
			sprintf(str, "%ld", j);
			eof_log("arpeggio = ",1);
			eof_log(str,1);
			sectionptr = &eof_song->pro_guitar_track[srctracknum]->arpeggio[j];
			long tempendpos = 0;
			for(i = 0; i < note; i++)
			{
				sprintf(str, "%ld", i);
				eof_log("note = ",1);
				eof_log(str,1);
				long notepos = eof_song->pro_guitar_track[srctracknum]->note[i]->pos;
				if((notepos >= sectionptr->start_pos) && (notepos <= sectionptr->end_pos))
				{	//If the note is in this arpeggio section
					eof_log("ghey4",1);
					long after = eof_track_fixup_next_note(eof_song, eof_selected_track, i);
					if(after > 0)
					{
						tempendpos = eof_get_note_max_length(eof_song,eof_selected_track,i,1);
						tempendpos += notepos;
					}
					else
					{
					...
					sprintf(str, "%ld", tempendpos);
					eof_log("end_pos = ",1);
					eof_log(str,1);
				}
			}
			sectionptr->end_pos = tempendpos;
		}
	}
